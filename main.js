...
/* ---- interactive canvas background (connected particles) ---- */
const canvas=document.getElementById('bg'),ctx=canvas.getContext('2d');
let W,H,pts,mx=-999,my=-999;
function resize(){W=canvas.width=innerWidth;H=canvas.height=innerHeight;const n=Math.min(90,Math.floor(W*H/16000));pts=Array.from({length:n},()=>({x:Math.random()*W,y:Math.random()*H,vx:(Math.random()-.5)*.4,vy:(Math.random()-.5)*.4}));}
...
function draw(){
  ...
  for(const p of pts){p.x+=p.vx;p.y+=p.vy;if(p.x<0||p.x>W)p.vx*=-1;if(p.y<0||p.y>H)p.vy*=-1;
    ...
    if(d<160){p.x+=dx/d*.6;p.y+=dy/d*.6;}
    ctx.beginPath();ctx.arc(p.x,p.y,2,0,7);ctx.fillStyle='rgba(124,92,255,.5)';ctx.fill();}
  for(let i=0;i<pts.length;i++)for(let j=i+1;j<pts.length;j++){const a=pts[i],b=pts[j],d=Math.hypot(a.x-b.x,a.y-b.y);if(d<130){ctx.strokeStyle='rgba(34,211,238,'+(1-d/130)*.25+')';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();}}
  requestAnimationFrame(draw);
}
draw();

/* ---- magnetic + tilt ---- */
document.querySelectorAll('.mag').forEach(b=>{b.addEventListener('mousemove',e=>{const r=b.getBoundingClientRect();const x=e.clientX-r.left-r.width/2,y=e.clientY-r.top-r.height/2;b.style.transform=`translate(${x*.25}px,${y*.35}px)`;});b.addEventListener('mouseleave',()=>b.style.transform='');});
